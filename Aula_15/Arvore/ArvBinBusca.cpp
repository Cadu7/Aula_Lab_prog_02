...
void ArvBinBusca::removeFolhaVal(int valor) {

    auxFolhaRemove( raiz, valor);
}

void ArvBinBusca::auxFolhaRemove(NoArv *p, int valor) {

    if(p == NULL){
        cout << "Valor n]ao encontrado" << endl;
    }
    else if(p->getInfo() == valor)
        removeFolha(p);
    else if(valor < p->getInfo())
        auxFolhaRemove(p->getEsq(), valor);
    else
        auxFolhaRemove(p->getDir(), valor);
}
